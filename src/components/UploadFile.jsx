...
const UploadFile = () => {
  ...
  return (
    <div className="admin-page d-flex flex-column align-items-center">
      {
          loading && <div style={{marginTop:"15%"}} className="position-absolute" >
            <RotatingLines
              strokeColor="black"
              strokeWidth="4"
              animationDuration="1"
      ...
      <div className="file-upload d-flex flex-column p-5">
        ...
        <div className="d-flex flex-column m-4">
          ...
          <input onChange={(e) => setPdf(e.target.files[0])} type="file" className="opacity-1" name="" id="" />
        </div>
        {pdf && <p className="mx-auto" >{pdf.name}</p>}
        <button onClick={uploadFile} className="w-50 mx-auto btn btn-primary">Upload</button>
      </div>
    </div>
  )
}
